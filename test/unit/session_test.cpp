...
BOOST_AUTO_TEST_CASE(async_send_and_receive)
{
    ...
    auto client = [&]() -> asio::awaitable<void>
    {
        ...
        {
            auto req_seq_num = co_await session.async_send(smpp::submit_sm{});
            auto [pdu, seq_num, status] = co_await session.async_receive();
            BOOST_CHECK(
                std::get<smpp::submit_sm_resp>(pdu) == smpp::submit_sm_resp{});
            BOOST_CHECK(status == smpp::command_status::rok);
            BOOST_CHECK_EQUAL(req_seq_num, seq_num);
            BOOST_CHECK(seq_num == 1);
        }
        {
            auto req_seq_num = co_await session.async_send(smpp::deliver_sm{});
            auto [pdu, seq_num, status] = co_await session.async_receive();
            BOOST_CHECK(
                std::get<smpp::deliver_sm_resp>(pdu) ==
        ...
        {
            auto req_seq_num =
                co_await session.async_send(smpp::bind_transceiver{});
            auto [pdu, seq_num, status] = co_await session.async_receive();
            BOOST_CHECK(
                std::get<smpp::bind_transceiver_resp>(pdu) ==
    ...
    auto server = [&]() -> asio::awaitable<void>
    {
        ...
        auto session = smpp::session{ co_await acceptor.async_accept() };

        {
            ...
            co_await session.async_send(
                smpp::submit_sm_resp{}, seq_num, smpp::command_status::rok);
        }
        {
            auto [pdu, seq_num, status] = co_await session.async_receive();
            ...
            co_await session.async_send(
                smpp::deliver_sm_resp{},
                seq_num,
                smpp::command_status::ralybnd);
        }
        {
            auto [pdu, seq_num, status] = co_await session.async_receive();
            BOOST_CHECK(
                std::get<smpp::bind_transceiver>(pdu) ==
...
BOOST_AUTO_TEST_CASE(enquire_link_timeout)
{
    ...
    auto client = [&]() -> asio::awaitable<void>
    {
        ...
        co_await socket.async_connect({ asio::ip::tcp::v4(), 2775 });
        auto session =
            smpp::session{ std::move(socket), std::chrono::seconds{ 1 } };
    ...
    auto server = [&]() -> asio::awaitable<void>
    {
        ...
        auto session = smpp::session{ co_await acceptor.async_accept() };
        auto timer =
            asio::steady_timer{ executor, std::chrono::milliseconds{ 2500 } };
