...
public:
    ...
    /**
     * Constructs an oparam from buffer and is intended to be used by pdu
     * deserializer.
     *
    ...
    {
        auto deserialize_u16 = [](std::span<const uint8_t, 2> buf) -> uint16_t
        { return buf[0] << 8 | buf[1]; };

        while(buf->size() >= 4)
        {
            auto tag =
                static_cast<oparam_tag>(deserialize_u16(buf->subspan<0, 2>()));
            auto val_length = deserialize_u16(buf->subspan<2, 2>());

            if(val_length > buf->size() - header_length)
                throw std::length_error{
                    "oparam val length is bigger than available buf"
                };

            auto val_buf = buf->subspan(header_length, val_length);
            oparams_.emplace(
                tag, std::string{ val_buf.begin(), val_buf.end() });
    ...
    bool
    operator==(const oparam&) const = default;

    /// Serialize the oparam
    /**
     * This function serialize the oparam and is intended to be used by pdu
     * serializer.
     *
     * @param vec The vector that optional parameters would be appended to.
     */
    void
    serialize(std::vector<uint8_t>* vec) const
    {
     ...
     *
     * @return A boolean indicating if there was an optional parameter with this
     * oparam_tag.
     *
     * @param tag The oparam_tag to be erased.
     */
    bool
    erase(oparam_tag tag)
    {
    ...
    /**
     * This function finds whether an optional parameter with the given
     * oparam_tag exists.
     *
     * @return A boolean indicating if there was an optional parameter with this
     * oparam_tag.
     ...
     */
    const std::string&
    get_as_string(oparam_tag tag) const
    {
     ...
     */
    void
    set_as_string(oparam_tag tag, std::string val)
    {
        if(val.size() > 65535)
            throw std::length_error{
                "oparam value length is bigger than 65535"
            };
    ...
    /**
     * This function gets an optional parameter as an underlying type of
     * uint8_t.
     *
     * @throw std::runtime_error if optional parameter does not exist.
     *
    ...
    template<typename T>
    T
    get_as_enum_u8(oparam_tag tag) const
        requires(std::is_same_v<std::underlying_type_t<T>, uint8_t>)
    {
    ...
    /**
     * This function sets an optional parameter as an enum with an underlying
     * type of uint8_t. If a value with the same oparam_tag already exists it
     * will be replaced.
     *
    ...
    template<typename T>
    void
    set_as_enum_u8(oparam_tag tag, T val)
        requires(std::is_same_v<std::underlying_type_t<T>, uint8_t>)
    {
