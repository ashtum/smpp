...
{
    smpp::smsc_delivery_receipt smsc_delivery_receipt{
        smsc_delivery_receipt::no
    };
    smpp::sme_originated_ack sme_originated_ack{ sme_originated_ack::no };
    smpp::intermediate_notification intermediate_notification{
        intermediate_notification::no
    };

    bool
    operator==(const registered_delivery&) const = default;

    static registered_delivery
    from_u8(uint8_t value)
    {
    ...
    {
        return static_cast<uint8_t>(smsc_delivery_receipt) |
            static_cast<uint8_t>(sme_originated_ack) |
